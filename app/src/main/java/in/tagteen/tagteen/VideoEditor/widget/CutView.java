...
public class CutView extends View {
    ...
    private int dp1;
    private int widRatio = 0, heiRatio = 0;
...
//                invalidate();
    ...
    //设置裁剪框比例
    public void setCutRatio(int widthRatio, int heightRation) {
        widRatio = widthRatio;
        heiRatio = heightRation;
        ...
        if (widthRatio > heightRation) {
            ...
            if (rectWidth>rectHeight){
                rectBottom = measuredHeight - marginBottom;
                rectRight = rectHeight * widthRatio / heightRation;
            }else {
                rectRight = measuredWidth - marginRight;
                rectBottom = (rectWidth * heightRation / widthRatio) + marginTop;
            }

        } else if (widthRatio == heightRation) {
            int rectWidth = getRectWidth();
            int rectHeight = getRectHeight();
            if (rectWidth >= rectHeight) {
                rectRight = rectHeight;
                rectBottom = rectHeight + marginTop;
            } else {
                rectRight = rectWidth;
                rectBottom = rectWidth;
            }
    ...
    //裁剪框按比例缩放
    public void changeByRatio() {
        measuredHeight = getMeasuredHeight();
        measuredWidth = getMeasuredWidth();
        float rectWidth = rectRight - rectLeft;
        float rectHeight = rectBottom - rectTop;
        if (widRatio > 0 && !isMove) {

            if (isLeft || isRight) {
                rectBottom = rectTop + rectWidth * heiRatio / widRatio;
                float width = rectHeight * widRatio / heiRatio;
                if (rectRight > rectLeft + width) {
                    rectRight = rectLeft + width;
                }
            } else {
                rectRight = rectLeft + rectHeight * widRatio / heiRatio;
                float height = rectWidth * heiRatio / widRatio;
                if (rectBottom > rectTop + height) {
